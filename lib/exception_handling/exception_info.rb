...
EOF
    ...
    HONEYBADGER_CONTEXT_SECTIONS = [:timestamp, :error_class, :context, :server, :scm_revision, :notes, :user_details, :request, :session, :environment, :backtrace, :event_response]
    ...
    def enhanced_data_to_honeybadger_context
      ...
      data[:context] = @exception_context if @exception_context.present?
